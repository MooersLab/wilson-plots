...
def parse_truncate_log(log_file):
    ...
    try:
        ...
        print "Successfully extracted %d data points" % len(resolution_inv_sq)
        return resolution_inv_sq, ln_intensity
    
    except Exception as e:
        print "Error processing log file: %s" % e
        return None, None
...
def create_wilson_plot(resolution_inv_sq, ln_intensity, output_file):
    ...
    try:
        ...
        wilson_b = -2 * m
        print "Wilson B-factor: %.2f A^2" % wilson_b
        ...
        ax1.plot(x, fit_y, 'r-', linewidth=2, label='Wilson Fit (B = %.2f A^2)' % wilson_b)
        
        # Set up the primary axis (1/d²)
        ax1.set_xlabel(r'1/d$^2$ (A$^{-2}$)')
        ax1.set_ylabel(r'ln<I/f$^2$>')
        ...
        # Create tick labels
        tick_labels = ["%.1f" % r for r in tick_resolutions]
        ...
        print "Wilson plot saved to: %s" % output_file
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print "Usage: python wilsonplot.py <truncate_log_file> <output_image>"
        sys.exit(1)
